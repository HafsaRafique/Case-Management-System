...
// SQL query to fetch data from the "contacts" table with an additional join to retrieve clientID from the "assignment" table
$sql = "SELECT c.lawyerID, l.Name AS lawyer_name, c.courtID, a.caseID, a.clientID, cl.FullName AS client_name
        FROM contacts c
...
if ($result->num_rows > 0) {
    ...
    echo '<th>Action</th>'; // Add a column for the action (update)
    echo '</tr>';
    while($row = $result->fetch_assoc()) {
        ...
        echo '<td class="contacts-column">' . '<form method="post">
                                                <input type="hidden" name="lawyerID" value="' . $row["lawyerID"] . '">
                                                <input type="text" name="casestatus" value="' . $row["casestatus"] . '" required>
                                                <button type="submit">Update</button>
                                              </form>' . '</td>'; // Form for updating case status
        echo '</tr>';
    }
    echo '</table>';
} else {
...
// Handle the form submission for updating case status
if(isset($_POST['casestatus']) && isset($_POST['lawyerID'])) {
    ...
    // Update the case status in the contacts table
    $sql_update = "UPDATE contacts SET casestatus = ? WHERE lawyerID = ?";
    $stmt_update = $conn->prepare($sql_update);
    $stmt_update->bind_param("si", $casestatus, $lawyerID);
    
    if ($stmt_update->execute()) {
        echo "<script>alert('Case status updated successfully.')</script>";
    } else {
